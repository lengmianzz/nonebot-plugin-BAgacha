...
from nonebot.adapters.onebot.v11 import MessageEvent, GroupMessageEvent, GROUP_ADMIN, GROUP_OWNER
require("nonebot_plugin_apscheduler")
require("nonebot_plugin_saa")
from nonebot_plugin_apscheduler import scheduler
from nonebot_plugin_saa import MessageFactory, Text, Image, TargetQQGroup, enable_auto_select_bot
...
@matcher0.handle()
async def send_one_pulls_res(event: MessageEvent) -> NoReturn:
...
@matcher1.handle()
async def send_ten_pulls_res(event: MessageEvent) -> NoReturn:
...
@matcher2.handle()
async def send_two_hundred_pulls_res(event: MessageEvent) -> NoReturn:
...
async def _(event: GroupMessageEvent) -> NoReturn:
    white.append(event.group_id)
    
    msg = MessageFactory("已将本群加入更新的订阅名单!")
...
async def latest_update() -> NoReturn:
    ...
    if sorted(up_ids) != sorted(latested_up_ids):
        ...
        if white:
            ...
            for group in white:
                target = TargetQQGroup(group_id=group)
